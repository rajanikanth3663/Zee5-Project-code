...
resource "azurerm_cdn_frontdoor_endpoint" "this" {
  name                     = var.settings.endpoint.name
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.this.id
  enabled                  = var.settings.endpoint.enabled
}
...
resource "azurerm_cdn_frontdoor_route" "this" {
  ...
  name                          = each.key
  cdn_frontdoor_endpoint_id     = azurerm_cdn_frontdoor_endpoint.fde[each.value.frontdoor_endpoint_name].id
...
resource "azurerm_cdn_frontdoor_firewall_policy" "this" {
  name                              = var.settings.firewall.name
  resource_group_name               = var.settings.firewall.resource_group_name
  sku_name                          = azurerm_cdn_frontdoor_profile.this.sku_name
  enabled                           = var.settings.firewall.enabled
  mode                              = var.settings.firewall.mode
  redirect_url                      = var.settings.firewall.redirect_url
  custom_block_response_status_code = var.settings.firewall.custom_block_response_status_code
  custom_block_response_body        = var.settings.firewall.custom_block_response_body
  ...
  dynamic "managed_rule" {
    for_each = (var.settings.firewall.managed_rule == null) ? [] : var.settings.firewall.managed_rule
...
resource "azurerm_cdn_frontdoor_security_policy" "afd_waf_link" {
  name                     = azurerm_cdn_frontdoor_firewall_policy.this.name
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.this.id
